...
class MarketMixModel():
    ...
    def __init__(self, m = 2, n = 1, l = 5, year = 1, mcmc = 8000, path_len = 0.05, step_size = 0.01):
        #######
    ...
    def _get_para(self, phi):
        ...
        self.intercepts = phi[3*self.m:3*self.m+self.total_size].copy()
        self.beta_s = phi[3*self.m+self.total_size: 3*self.m+self.total_size+(self.m+self.n)*self.cluster].copy()
        ...
        self.variance0 = phi[3*self.m+self.total_size+(self.m+self.n)*self.cluster:3*self.m+self.total_size+(self.m+self.n)*self.cluster+1]
    ...
    def _compute_prior(self):
        ...
        log_alpha_star = self._log_normpdf_nelson(self.alpha_s_star, mu = 0, sigma = 0.5).sum()
        ###### k, shape
        ...
        log_intercepts = self._log_truncate_norm_pdf_nelson(self.intercepts, a = 0.5, b = 1.5, loc = 1, scale = 0.02).sum()
        ...
        log_beta_s = self._log_truncate_norm_pdf_nelson(self.beta_s, a = 0.5, b = 1.5, loc = 1, scale = 0.02).sum()
        ############# random effects
        #log_beta_s_random = tr_My2.logpdf(self.beta_s_random).sum()
        #log_beta_s_random = self._log_truncate_norm_pdf_nelson(self.beta_s_random, a = 0, b = 10, loc = 1, scale = 0.5).sum()

        
        ################ variance for random effects
        #log_variance_s = invgamma.logpdf(self.variance_s, a = 1, loc=0, scale=1).sum()
        #log_variance_s = self._log_invgammapdf_nelson(self.variance_s, a = 1, loc=0, scale=1).sum()
        ####### variance
        #log_variance0 = invgamma.logpdf(self.variance0, a = 1, loc=0, scale=1).sum()
        log_variance0 = self._log_invgammapdf_nelson(self.variance0, a = 1, loc=0, scale=1).sum()
        
        ######
        ss = np.array([log_alpha_star, log_k, log_lambda, log_intercepts, log_beta_s, log_variance0])
        if np.any(ss < -10000):
            final_results = math.inf
        else:
    ...
    def _log_invgammapdf_nelson(self, x, a = None, loc = 0, scale = None):
        if (x <= 0).any():
            return np.array([-10001])
        else:
        	y = (x - loc)/scale
    ...
    def GenerateY(self, phi = None, Y= None):
        ...
        cluster_size_0 = np.concatenate((np.array([0]), self.cluster_size)) ### (0, 3, 3)
        self.cluster_size_sum = np.cumsum(cluster_size_0) #### (0, 3, 6)
        self.total_size = self.cluster_size_sum[-1] ## 6
        
        ####
        self.num_product_store = Y.groupby(['cluster', 'store']).size().ravel()  ###### get the number of products per store (52, 52, 52, 52)
        self.tem_index_row = np.cumsum(self.num_product_store)[:-1]
        self.tol_product_size = int((Y.groupby(['cluster', 'store']).size()/self.w).ravel().sum()) #### get the number of individual product size
        self.Y = Y
        ...
        ###### 
        y_tt = norm.rvs(loc=mu_fi, scale=np.sqrt(self.variance0), random_state=2020)
        #ll = norm.logpdf(Y_Y, mu_fi, np.sqrt(self.variance0)).sum()
        ######## generated y and predicted y, use the mean
        return [y_tt, mu_fi]
