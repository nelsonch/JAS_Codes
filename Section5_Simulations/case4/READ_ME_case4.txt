run4.sh is the shell script that will kick off case4_run.py in background on a linux server. Please make sure to properly change the working directory and the executable python PATH. It is much handy to run on a server than a local laptop.

case4_run.py when finished will write out a csv file, "case4_results.csv" and the last row in the csv file are the RMSE values of the three methods reported in Table 3 in the main article. The computing environment I used are as follows.
