...
from MarketMix_HMC_2g_May_2021 import MarketMixModel
from MaxLikelihood_2g_May_2021 import LK_MLE

print('BEGIN_CODES')
########## initial values
m = 2; nn = 1; w = 104; g = 2
cluster = np.array([1])  #### 
store = np.array([1, 2]) ### each store has 2 products 
product = np.array([1]) #### 
...
######
tem = np.concatenate((cluster_ind, store_ind, product_ind, m51, m52, SI), 1)
Y = pd.DataFrame(tem, columns=np.array(['cluster', 'store', 'product', 'm51', 'm52', 'SI']))
############# true parameters 
three = np.array([0, 0, 0.2, 0.2, 0.3, 0.3]) ##### the first two 0 are at transformed scale and are actually 0.5 at the original scale.
intercepts = np.array([1, 1])
fixed = np.array([1, 1, 1])
variances = np.array([0.25, 0.25, 0.25])
random_s = np.tile(np.array([1, 1, 1]), g)
variances0 = np.array([0.25])
#init = np.concatenate((three, intercepts, fixed, variances0))
init = np.concatenate((three, intercepts, fixed, variances, random_s, variances0))
############## simulate data
fit = MarketMixModel(m = m, n = nn, l = 5, year = int(w/52)).GenerateY(init, Y)
#Y = Y.drop('Y', axis = 1)
Y['Y'] = fit[0]
print('DATA HAS BEEN SIMULATED')
Y.to_csv('case6_data.csv')
...
for k in range(K):
    ...
    np.random.seed(2020+2*k)
    v = np.random.uniform(low =0, high = 1, size = (m+nn))
    np.random.seed(2020+2*k)
    rf_beta = np.random.uniform(low =0.5, high=1.5, size = g*(m+nn))
    np.random.seed(2020+2*k)
    v0 = np.random.uniform(size = 1)
    phi0 = np.concatenate((alpha_s, k_s, lambda_s, intercepts, beta, v, rf_beta, v0))
    ######### get the object 
    ...
    fit1 = optimize.minimize(tem._negative_log_posterior, x0=phi0, method = 'L-BFGS-B', 
                             bounds=mybounds,
                            options={'gtol': 1e-05, 'eps': 1e-13, 'maxiter':100})
    if fit1.fun < t_bfgs:
        t_bfgs = fit1.fun
        op_s = fit1.x
    ...
    fit2 = optimize.minimize(tem._negative_log_posterior, x0=phi0, method = 'SLSQP', 
                             bounds=mybounds,
                            options={'ftol': 1e-05, 'eps': 1e-13, 'maxiter':100}) 
    if fit2.fun < t_sqp:
        t_sqp = fit2.fun
        op_sqp = fit2.x
...
res_bfgs = op_s
res_sqp = op_sqp
true = np.array([0.5, 0.5, 0.2, 0.2, 0.3, 0.3, 1, 1, 1, 1, 1, 0.25, 0.25, 0.25, 1, 1, 1, 1, 1, 1, 0.25])
...
####### Fit HMC 
fit = MarketMixModel(m = m, n = nn, l = 5, year = int(w/52), mcmc = 30000, path_len = 0.0010, step_size = 0.0005)
fit3 = fit.HMC(init, Y)
tra = fit3['tra']
tra_pd = pd.DataFrame(tra)

######## get the results from trajectory
t = (np.linspace(start=6000, stop=30000, num= int((30000 - 6000)/400 + 1)) - 1).astype(int)
index_samples = t[:-1]
...
case6_results = pd.DataFrame(rr, columns=['true', 'HMC', 'BFGS', 'SQP']) 

################
case6_results_pd = pd.DataFrame(case6_results)
case6_results_pd.to_csv('case6_results.csv')
