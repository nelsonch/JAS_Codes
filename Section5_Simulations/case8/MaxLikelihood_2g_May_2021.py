...
class LK_MLE():
    ...
    def __init__(self, Y = None, m = 7, n = 3, l = 5, year = 1):
        #######
    ...
    def _log_tnpdf_nelson(self, x, mu, sigma):
        wo = -mu/sigma
        C = 0.5*(1+erf(wo/np.sqrt(2)))
        cc = 1/(sigma*(1-C))
        return np.log(cc)-0.5*np.log(2*math.pi*sigma**2)-(x-mu)**2/2/sigma**2
