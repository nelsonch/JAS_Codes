...
############### plot 
########
tra_pd = pd.read_csv('case1_trajectory_after_burnin.csv').iloc[:,1:]
tra = tra_pd.values
#tra_pd.head(3)
...
t = (np.linspace(start=0, stop=24000, num= int((24000 - 0)/300 + 1)) - 1).astype(int)
index_samples = t[:-1]
...
sm.graphics.tsa.plot_acf(tem_hh[:, 7], lags=40, ax=ax[0], fft=True, adjusted=False, title='(a)')
ax[0].set_xlabel('Lag')
sm.graphics.tsa.plot_acf(tem_hh[:, 8], lags=40, ax=ax[1], fft=True, adjusted=False, title='(b)')
ax[1].set_xlabel('Lag')
plt.savefig('case1_acf.pdf')
plt.show()
...
plt.subplot(121)
plt.plot(np.arange(tem_hh.shape[0]), tem_hh[:, 7])
...
plt.subplot(122)
plt.hist(tem_hh[:, 7], orientation='horizontal',bins=20)
plt.axhline(y=1.0, color='r', linestyle='-')
plt.yticks(np.arange(0.95, 1.05, 0.05))
plt.title('(b)')
plt.xlabel('Frequency')
plt.tight_layout()
plt.savefig('case1_beta1.pdf')
plt.show()
